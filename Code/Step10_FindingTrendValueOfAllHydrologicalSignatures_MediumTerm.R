...
write.csv(tau_results_pivoted,file.path(file_Path_Variable_O, "MediumSubset_AnnualTrendAnalysis_HydrologicalSignatures.csv") )

output_path <- "/Users/ahowl/Desktop/KGS Data analysis/TooBigForGithub_Steps_Workflow_Sept17/Output/TrendResults_MediumTerm"
...
create_kendall_tau_maps <- function(data) {
  tau_columns <- grep("_tau$", names(data), value = TRUE)
  p_value_columns <- str_replace(tau_columns, "_tau$", "_p_value")
    maps <- list()
  ...
  for (i in seq_along(tau_columns)) {
    ...
    # Add the map to the list with a descriptive name
    map_name <- paste0(tau_col, "_Kendall_Tau_Map")
    maps[[map_name]] <- map
  }
...
tau_maps <- create_kendall_tau_maps(tau_results_pivoted)

print(tau_maps[["Max_7DayStreamflow_Fall_SON_MMD_tau_Kendall_Tau_Map"]])

output_path <- "/Users/ahowl/Desktop/KGS Data analysis/TooBigForGithub_Steps_Workflow_Sept17/Output/TrendResults_MediumTerm_UsingAnotherWay"
for (map_name in names(tau_maps)) {
  ggsave(
    filename = file.path(output_path, paste0(map_name, ".png")),
    plot = tau_maps[[map_name]],
    width = 10,
    height = 8,
    dpi = 300
  )
