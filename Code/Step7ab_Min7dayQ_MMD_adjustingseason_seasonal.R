...
# 
# 
# LongTerm_Min_7DayStreamflow_Seasonal <- LongTerm_StreamflowData_7DayAvg %>%
#   group_by(site_no, Year, Season) %>%
#   summarise(Min_7DayStreamflow = min(Streamflow_7DayAvg, na.rm = TRUE)) %>%
#   spread(Season, Min_7DayStreamflow, fill = NA) %>%  # Spread data by season
#   rename(Min_7DayStreamflow_Winter = Winter, 
#          Min_7DayStreamflow_Spring = Spring, 
#          Min_7DayStreamflow_Summer = Summer, 
#          Min_7DayStreamflow_Fall = Fall) %>%
#   ungroup()
# 
# 
...
assign_season_and_year <- function(date) {
  ...
  if (month == 12) {
    return(list(season = "Winter", adjusted_year = year + 1))
  } else if (month %in% c(1, 2)) {
    return(list(season = "Winter", adjusted_year = year))
  } else if (month %in% c(3, 4, 5)) {
    return(list(season = "Spring", adjusted_year = year))
  } else if (month %in% c(6, 7, 8)) {
    return(list(season = "Summer", adjusted_year = year))
  } else {
    return(list(season = "Fall", adjusted_year = year))
  }
}
...
LongTerm_StreamflowData_7DayAvg <- LongTerm_StreamflowData %>%
  ...
  mutate(Year = year(Date),
         ...
         adjusted_year = sapply(season_data, `[[`, "adjusted_year")) %>%  # Assign seasons and adjust years
  arrange(Date) %>%
  mutate(Streamflow_7DayAvg = rollapplyr(mean_streamflow_mm_per_day, 
                                         width = 7, 
                                         FUN = mean, 
...
MediumTerm_Min_7DayStreamflow_Seasonal <- MediumTerm_StreamflowData_7DayAvg %>%
  group_by(site_no, adjusted_year, Season) %>%
  summarise(Min_7DayStreamflow = min(Streamflow_7DayAvg, na.rm = TRUE)) %>%
  spread(Season, Min_7DayStreamflow, fill = NA) %>%  # Spread data by season
  rename(Min_7DayStreamflow_Winter = Winter, 
...
# Adjusted function to assign season and adjust year for December
assign_season_and_year <- function(date) {
  month <- as.numeric(format(date, "%m"))
  year <- year(date)
...
AllYear_Min_7DayStreamflow_Seasonal <- AllYear_StreamflowData_7DayAvg %>%
  group_by(site_no, adjusted_year, Season) %>%
  summarise(Min_7DayStreamflow = min(Streamflow_7DayAvg, na.rm = TRUE)) %>%
  spread(Season, Min_7DayStreamflow, fill = NA) %>%  # Spread data by season
  rename(Min_7DayStreamflow_Winter = Winter, 
...
AllYear_StreamflowData_Annual <- readRDS(file.path(file_Path_Variable_O, "AllYear_StreamflowData_Annual_Min7_Max7_step7.rds"))
MediumTerm_StreamflowData_Annual <- readRDS(file.path(file_Path_Variable_O, "MediumTerm_StreamflowData_Annual_Min7_Max7_step7.rds"))
LongTerm_StreamflowData_Annual <- readRDS(file.path(file_Path_Variable_O, "LongTerm_StreamflowData_Annual_Min7_Max7_step7.rds"))

AllYear_Min_7DayStreamflow_Seasonal<- AllYear_Min_7DayStreamflow_Seasonal %>% rename(Year=adjusted_year)
MediumTerm_Min_7DayStreamflow_Seasonal<- MediumTerm_Min_7DayStreamflow_Seasonal %>% rename(Year=adjusted_year)
LongTerm_Min_7DayStreamflow_Seasonal<- LongTerm_Min_7DayStreamflow_Seasonal %>% rename(Year=adjusted_year)

AllYear_StreamflowData_Annual_Min7_Seasonal<- AllYear_StreamflowData_Annual %>% 
  left_join(AllYear_Min_7DayStreamflow_Seasonal, by=c('site_no','Year'))
AllYear_StreamflowData_Annual_Min7_Seasonal




MediumTerm_StreamflowData_Annual_Min7_Seasonal<- MediumTerm_StreamflowData_Annual %>% 
  left_join(MediumTerm_Min_7DayStreamflow_Seasonal, by=c('site_no','Year'))
MediumTerm_StreamflowData_Annual_Min7_Seasonal



LongTerm_StreamflowData_Annual_Min7_Seasonal<- LongTerm_StreamflowData_Annual %>% 
  left_join(LongTerm_Min_7DayStreamflow_Seasonal, by=c('site_no','Year'))
LongTerm_StreamflowData_Annual_Min7_Seasonal


length(unique(LongTerm_StreamflowData_Annual_Min7_Seasonal$site_no))
length(unique(MediumTerm_StreamflowData_Annual_Min7_Seasonal$site_no))


# 
saveRDS(AllYear_StreamflowData_Annual_Min7_Seasonal,file.path(file_Path_Variable_O, "AllYear_StreamflowData_Annual_Min7_Max7_Min7Seasonal_adjustingseason_step7.rds"))
saveRDS(MediumTerm_StreamflowData_Annual_Min7_Seasonal,file.path(file_Path_Variable_O, "MediumTerm_StreamflowData_Annual_Min7_Max7_Min7Seasonal_adjustingseason_step7.rds"))
saveRDS(LongTerm_StreamflowData_Annual_Min7_Seasonal,file.path(file_Path_Variable_O, "LongTerm_StreamflowData_Annual_Min7_Max7_Min7Seasonal_adjustingseason_step7.rds"))
# 
# 
