...
for (i in 1:nrow(AllYear_StreamflowData)) {
  ...
  # Sum the total drought deficit for each station  ##Cfd convert 
...
for (Threshold in thresholds) {
  for (Type in types) {
    ...
    ggsave(filename = file.path(file_Path_Variable_O, paste0("Step9_DroughtDeficit_Threshold_Long", Threshold, "_", Type, ".jpg")), 
           plot = threshold_values_df_Map, width = 8, height = 6, dpi = 300)
...
ggsave(filename = file.path(file_Path_Variable_O, "Combined_Fixed_Thresholds_Step8_Long.jpg"), 
       plot = combined_fixed_plots, width = 20, height = 12, dpi = 300)

ggsave(filename = file.path(file_Path_Variable_O, "Combined_Variable_Thresholds_Step8_Long.jpg"), 
